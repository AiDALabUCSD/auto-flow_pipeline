...
def generate_gif_for_slice(slice_idx, input_array, pred_array, timepoint=3):
    """
    ...
    axes[0].imshow(input_array[timepoint, :, :, slice_idx, 0], cmap='gray')
    ...
    for i in range(8):
        axes[i + 1].imshow(input_array[timepoint, :, :, slice_idx, 0], cmap='gray')
        axes[i + 1].imshow(pred_array[timepoint, :, :, slice_idx, i], cmap='jet', alpha=0.3)
...
def generate_gif(patient_name, base_output_folder, input_array=None, pred_array=None, timepoint=3, n_jobs=-1):
    ...
    if pred_array is None:
        pred_nii_path = os.path.join(path_to_patient, 'pred_from_locnet.nii.gz')
    ...
    gif_path = os.path.join(path_to_patient, 'pred_from_locnet.gif')

    logger.info("Generating GIF...")
    num_slices = input_array.shape[3]
    frames = Parallel(n_jobs=n_jobs)(delayed(generate_gif_for_slice)(i, input_array, pred_array, timepoint) for i in tqdm(range(num_slices), desc=f"Generating GIF for {patient_name}"))
